...
global LineWidth FontSize...
    ...
    BeadSpringOrMeso RawDataFileName ForceDataFileName TimeStretchDataFileName...
    CalculateForce CalculateEndtoEnd...
    CalculateAlignment...
    CurrentFolder OutputFolder BondType...
    OutputAtom OutputBond OutputSuccess SuccessfulSamples
    ...
    for m=1:length(kbTs)
     for n=1:length(bs)
      for o=1:length(dts)
        ...
        %% Keep ONLY successfully completed LAMMPS cases
        % A successful case must have the explicit success marker written by
        % RunSimulation plus both non-empty pull-stage dump files.  Failed or
        % partial runs are never passed to raw-data compilation or averaging.
        SuccessMask = false(size(PackageTemp,1),1);
        for q=1:size(PackageTemp,1)
            Sample_q = PackageTemp(q,1);
            InputScript(Sample_q,Np,D,N_Kuhn,stiffness,kbT,b,dt);
            SetDirAndFileNames;

            atomOK = isfile(OutputAtom);
            bondOK = isfile(OutputBond);
            if atomOK
                tmp = dir(OutputAtom);
            ...
            SuccessMask(q) = isfile(OutputSuccess) && atomOK && bondOK;
            if ~SuccessMask(q)
                fprintf('SKIP: sample %d was not a successful complete LAMMPS run.\n',Sample_q);
            end
        end
        PackageTemp = PackageTemp(SuccessMask,:);
        SuccessfulSamples = PackageTemp(:,1)';

        if isempty(PackageTemp)
            % Do not allow an old ensemble file to survive when there are no
            % currently successful cases for this parameter combination.
            ...
            fprintf('No successful cases for N=%d, stiffness=%.4g. No force data will be produced.\n',...
                N_Kuhn,stiffness);
        end
...
if CalculateForce && (~isfile(ForceDataFileName) || OverrideCompute)
    ...
    Forces.successful_samples = SuccessfulSamples;
...
rs = BondDataTemp(:,6:8);
rnet = sum(rs,1);
% Normalized chain extension: r* = r/(N*b)
stretch = vecnorm(rnet,2,2)/(N_Kuhn*b);

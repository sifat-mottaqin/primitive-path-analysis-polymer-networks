...
ShowFiguresDebug = 0;   % no intermediate/debug figures
LengthConversion = LC;
DamperConversion = DC;
...
L = ['\nvariable    cut_dyn equal ',num2str(1.15*Nb,'%.2e')]; Assembled = [Assembled,L];
...
% Keep the ORIGINAL generated chain at r*=0.95.  The preparation/push
% stage therefore starts at 0.95 and moves inward; it never stretches the
% initial chain beyond r*=1.  Only the subsequent pull reaches r*=1.15.
rstar_initial = 0.95;
rstar_final   = 1.15;
npts = 2*round(Nb/b*1.25);
push_distances = flipud((linspace(b,rstar_initial*Nb,npts))');
pull_distances = (linspace(b,rstar_final*Nb,npts))';
incr_fact = 10;
nhold = 8e4*incr_fact;    % number of hold steps at each pull position
nload = 10;               % retained from original workflow
thold = nhold*dt;
tload = nload*dt;
no_holds = length(pull_distances);
no_loads = no_holds-1;
...
% IMPORTANT: no atom/bond dumps are active during the preparation/push
% stage.  Force-vs-r* data are written only during the final pull stage.
...
L = '\n# Prepare the chain: push inward without recording force data'; Assembled = [Assembled,L];
if BeadSpringOrMeso==0
    end_bead = N_Kuhn+1;
elseif BeadSpringOrMeso==1
    end_bead = 2;
...
for i=1:length(push_distances)
    new_x_position = push_distances(i);
...
% Start a clean clock for the measured pull stage.  The raw dump files
% therefore contain ONLY the pull from r*=1/N to r*=1.15.
L = '\nreset_timestep 0'; Assembled = [Assembled,L];
L = '\n'; Assembled = [Assembled,L];
L = '\n# Compute/output bond information ONLY during the pull'; Assembled = [Assembled,L];
...
Assembled = [Assembled,L];
L = '\n'; Assembled = [Assembled,L];
L = '\n# Pull outward and record force data'; Assembled = [Assembled,L];

for i=1:length(pull_distances)
    new_x_position = pull_distances(i);
    L = ['\nset  atom   ',num2str(end_bead),...     % Set the end atom at the next pull position
        '   x   ',num2str(new_x_position,'%.4f'),...
        '   y   ',num2str(0,'%.1f'),...
        '   z   ',num2str(0,'%.1f')]; Assembled = [Assembled,L];
...
% The generated chain still starts at r*=0.95 (see InitiateGrid).
% Use a larger cubic box so the measured pull to r*=1.15 remains safely
% inside the periodic box instead of wrapping across the x boundary.
Lx = 3*N_Kuhn*b;
Ly = Lx;
Lz = Lx;
